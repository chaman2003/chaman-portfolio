...
const STOP_WORDS = new Set([
  'the', 'and', 'for', 'with', 'from', 'that', 'this', 'into', 'about', 'what', 'which', 'your', 'you',
  'how', 'can', 'are', 'was', 'were', 'have', 'has', 'had', 'who', 'him', 'her', 'his', 'their', 'them'
]);
...
function inferQueryIntents(query, tokens) {
  ...
  return {
    hiring: hasAny(['hire', 'hiring', 'contact', 'reach', 'available', 'internship', 'job', 'collab', 'collaboration']),
...
function pickContext(query, docs, topK = 8) {
  ...
  const scored = docs.map((rawDoc, index) => {
    ...
    if (intents.hiring && /(contact|email|phone|linkedin|hire|hiring|collaboration|internship|available)/.test(d)) {
      score += 8;
    }
